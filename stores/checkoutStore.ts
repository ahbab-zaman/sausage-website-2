...
export const useCheckoutStore = create<CheckoutStore>((set, get) => ({
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        set({
          ...
          selectedShippingMethod: firstShippingMethod,
          isLoading: false
        });
        return { success: true };
      }
  ...
  // Set payment method (local state only)
  ...
  // FIXED: Clear time slots before fetching new ones
  clearTimeSlots: () => {
    set({
      timeSlots: [],
      selectedTimeSlot: null,
  ...
  fetchTimeSlots: async (date) => {
    ...
    set({
      isLoading: true,
      error: null,
      timeSlots: [], // FIXED: Clear immediately
      selectedTimeSlot: null
    });
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        console.log(`✅ Fetched ${slots.length} time slots for ${date}`);
  ...
  reset: () => {
    // Clear API caches
    checkoutApiClient.clearAllCaches();
