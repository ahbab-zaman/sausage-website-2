...
//           timeSlots: slots,
//           selectedDate: date,
//           selectedTimeSlot: null, // Reset selected slot
//           isLoading: false,
//           error: null // Clear error when successful
//         });
//         return { success: true };
//       }
...
//   // Set error from outside the store
  ...
  fetchPaymentMethods: async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        // FIXED: Auto-select first shipping method if available
        const firstShippingMethod =
          flattenedMethods.length > 0 ? flattenedMethods[0].quote[0].code : null;

        set({
          paymentMethods: res.data.payment_methods || [],
          shippingMethods: flattenedMethods,
          selectedShippingMethod: firstShippingMethod, // Auto-select first method
          isLoading: false
        });
        return { success: true };
      }
